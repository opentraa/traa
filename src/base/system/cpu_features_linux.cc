...
#define TRAA_GLIBC_PREREQ(a, b) __GLIBC_PREREQ(a, b)
#else
#define TRAA_GLIBC_PREREQ(a, b) 0
#endif

#if TRAA_GLIBC_PREREQ(2, 16)
#include <sys/auxv.h>
#else
#include <errno.h>
#include <fcntl.h>
...
#if defined(TRAA_ARCH_ARM_FAMILY)
#include <asm/hwcap.h>
...
#endif // TRAA_GLIBC_PREREQ(2, 16)
#if defined(__aarch64__)
  (void)platform;
  architecture = 8;
  if ((hwcap & HWCAP_FP) != 0)
    result |= CPU_FEATURE_ARM_VFPV3;
  if ((hwcap & HWCAP_ASIMD) != 0)
    result |= CPU_FEATURE_ARM_NEON;
#else
  if (platform != NULL) {
...
#endif
  if (architecture >= 7)
    result |= CPU_FEATURE_ARM_ARMV7;
  if (architecture >= 6)
    result |= CPU_FEATURE_ARM_LDREXSTREX;
  return result;
}

} // namespace base
} // namespace traa
#endif // TRAA_ARCH_ARM_FAMILY
