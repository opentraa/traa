...
bool base64::is_base64_encoded(std::string_view str) {
...
size_t base64::get_next_quantum(decode_flags parse_flags, bool illegal_pads, const char *data,
...
bool base64::decode_from_array(const char *data, size_t len, decode_flags flags,
...
bool base64::decode_from_array(const char *data, size_t len, decode_flags flags,
  ...
  return decode_from_array_template<vector<char>>(data, len, flags, result, data_used);
}

bool base64::decode_from_array(const char *data, size_t len, decode_flags flags,
                               vector<uint8_t> *result, size_t *data_used) {
  return decode_from_array_template<vector<uint8_t>>(data, len, flags, result, data_used);
}

template <typename T>
bool base64::decode_from_array_template(const char *data, size_t len, decode_flags flags, T *result,
                                        size_t *data_used) {
  TRAA_DCHECK(result);
  TRAA_DCHECK_LE(flags, (DO_PARSE_MASK | DO_PAD_MASK | DO_TERM_MASK));

  const decode_flags parse_flags = flags & DO_PARSE_MASK;
  const decode_flags pad_flags = flags & DO_PAD_MASK;
  const decode_flags term_flags = flags & DO_TERM_MASK;
  TRAA_DCHECK_NE(0, parse_flags);
  TRAA_DCHECK_NE(0, pad_flags);
  TRAA_DCHECK_NE(0, term_flags);
  ...
  while (dpos < len) {
    size_t qlen =
        get_next_quantum(parse_flags, (DO_PAD_NO == pad_flags), data, len, &dpos, qbuf, &padded);
