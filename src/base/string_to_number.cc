...
template <typename T> T string_to(const char *str, char **str_end);

template <> inline float string_to(const char *str, char **str_end) {
  return std::strtof(str, str_end);
}

template <> inline double string_to(const char *str, char **str_end) {
  return std::strtod(str, str_end);
}

template <> inline long double string_to(const char *str, char **str_end) {
...
template <typename T> std::optional<T> parse_floating_point(std::string_view str) {
  ...
  char *end = nullptr;
  errno = 0;
  const T value = string_to<T>(str_str.c_str(), &end);
  if (end == str_str.c_str() + str_str.size() && errno == 0) {
    return value;
  }
  return std::nullopt;
...
template std::optional<float> parse_floating_point(std::string_view str);
template std::optional<double> parse_floating_point(std::string_view str);
template std::optional<long double> parse_floating_point(std::string_view str);
