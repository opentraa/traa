...
// NOTE: The k_ascii_property_bits table used within this code was generated by
...
// zero-initialized.
const unsigned char k_ascii_property_bits[256] = {
...
const char k_ascii_to_lower[256] = {
...
// identity of the passed character.
const char k_ascii_to_upper[256] = {
  '\x00', '\x01', '\x02', '\x03', '\x04', '\x05', '\x06', '\x07',
  '\x08', '\x09', '\x0a', '\x0b', '\x0c', '\x0d', '\x0e', '\x0f',
  '\x10', '\x11', '\x12', '\x13', '\x14', '\x15', '\x16', '\x17',
  '\x18', '\x19', '\x1a', '\x1b', '\x1c', '\x1d', '\x1e', '\x1f',
...
template <bool ToUpper> constexpr bool ascii_in_az_range(unsigned char c) {
  constexpr unsigned char k_sub = (ToUpper ? 'a' : 'A') - SCHAR_MIN;
  constexpr signed char k_threshold = SCHAR_MIN + 26; // 26 = alphabet size.
  // Using unsigned arithmetic as overflows/underflows are well defined.
  unsigned char u = c - k_sub;
  // Using signed cmp, as SIMD unsigned cmp isn't available in many platforms.
  return static_cast<signed char>(u) < k_threshold;
}

// Force-inline so the compiler won't merge the short and long implementations.
template <bool ToUpper>
HEDLEY_ALWAYS_INLINE constexpr void ascii_str_case_fold_impl(char *p, size_t size) {
  ...
  constexpr unsigned char k_ascii_case_bit_flip = 'a' ^ 'A';

  for (size_t i = 0; i < size; ++i) {
    unsigned char v = static_cast<unsigned char>(p[i]);
    v ^= ascii_in_az_range<ToUpper>(v) ? k_ascii_case_bit_flip : 0;
    p[i] = static_cast<char>(v);
  }
}

// The string size threshold for starting using the long string version.
constexpr size_t k_case_fold_threshold = 16;

// No-inline so the compiler won't merge the short and long implementations.
template <bool ToUpper>
HEDLEY_NEVER_INLINE constexpr void ascii_str_case_fold_long(char *p, size_t size) {
  HEDLEY_ASSUME(size >= k_case_fold_threshold);
  ascii_str_case_fold_impl<ToUpper>(p, size);
}

// Splitting to short and long strings to allow vectorization decisions
// to be made separately in the long and short cases.
template <bool ToUpper> constexpr void ascii_str_case_fold(char *p, size_t size) {
  size < k_case_fold_threshold ? ascii_str_case_fold_impl<ToUpper>(p, size)
                               : ascii_str_case_fold_long<ToUpper>(p, size);
}

static constexpr size_t validate_ascii_casefold() {
  constexpr size_t k_num_chars = 1 + CHAR_MAX - CHAR_MIN;
  size_t incorrect_index = 0;
  char lowered[k_num_chars] = {};
  char uppered[k_num_chars] = {};
  for (unsigned int i = 0; i < k_num_chars; ++i) {
    uppered[i] = lowered[i] = static_cast<char>(i);
  }
  ascii_str_case_fold<false>(&lowered[0], k_num_chars);
  ascii_str_case_fold<true>(&uppered[0], k_num_chars);
  for (size_t i = 0; i < k_num_chars; ++i) {
    ...
    if (uppered[i] != ch_upper || lowered[i] != ch_lower) {
      incorrect_index = i > 0 ? i : k_num_chars;
      break;
    }
  }
...
static_assert(validate_ascii_casefold() == 0, "error in case conversion");

} // namespace ascii

void ascii_str_to_lower(std::string *s) { return ascii_str_case_fold<false>(&(*s)[0], s->size()); }

void ascii_str_to_upper(std::string *s) { return ascii_str_case_fold<true>(&(*s)[0], s->size()); }
