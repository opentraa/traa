...
#if defined(TRAA_OS_WIN)
  if (options.allow_wgc_window_capturer() && is_wgc_supported(capture_type::window)) {
...
#if defined(TRAA_OS_WIN)
  if (options.allow_wgc_screen_capturer() && is_wgc_supported(capture_type::screen)) {
...
desktop_capturer::create_generic_capturer(const desktop_capture_options &options) {
  ...
  if (options.allow_pipewire() && capture_utils::is_running_under_wayland()) {
    capturer = std::make_unique<BaseCapturerPipeWire>(options, capture_type::any_content);
  }
