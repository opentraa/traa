...
#include "base/devices/screen/cropping_window_capturer.h"
...
#include "base/devices/screen/win/wgc_capturer_win.h"
...
DelegatedSourceListController *DesktopCapturer::GetDelegatedSourceListController() {
  return nullptr;
}
...
DesktopCapturer::CreateWindowCapturer(const DesktopCaptureOptions &options) {
#if defined(RTC_ENABLE_WIN_WGC)
...
// static
std::unique_ptr<DesktopCapturer>
DesktopCapturer::CreateScreenCapturer(const DesktopCaptureOptions &options) {
#if defined(RTC_ENABLE_WIN_WGC)
...
// static
std::unique_ptr<DesktopCapturer>
DesktopCapturer::CreateGenericCapturer(const DesktopCaptureOptions &options) {
  std::unique_ptr<DesktopCapturer> capturer;

#if defined(WEBRTC_USE_PIPEWIRE)
  if (options.allow_pipewire() && DesktopCapturer::IsRunningUnderWayland()) {
    capturer = std::make_unique<BaseCapturerPipeWire>(options, CaptureType::kAnyScreenContent);
  }
