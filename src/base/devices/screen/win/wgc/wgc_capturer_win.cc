...
void log_dirty_region_support() {
  ...
  hr = create_hstring(k_dirty_region_mode, wcslen(k_dirty_region_mode), &dirty_region_mode);
  if (FAILED(hr)) {
  ...
  hr = create_hstring(k_wgc_session_type, wcslen(k_wgc_session_type), &wgc_session_type);
  if (SUCCEEDED(hr)) {
...
wgc_capturer_win::wgc_capturer_win(const desktop_capture_options &options,
                                   ...
                                   std::unique_ptr<source_enumerator> source_enumerator,
...
bool wgc_capturer_win::is_wgc_supported(capture_type capture_type) {
  ...
  hr = create_hstring(k_api_contract, wcslen(k_api_contract), &api_contract);
  if (FAILED(hr))
    return false;
...
wgc_capturer_win::create_raw_window_capturer(const desktop_capture_options &options,
  ...
  return std::make_unique<wgc_capturer_win>(
      options, std::make_unique<wgc_capture_source_factory>(),
      std::make_unique<source_enumerator>(options.enumerate_current_process_windows()),
...
wgc_capturer_win::create_raw_screen_capturer(const desktop_capture_options &options) {
  return std::make_unique<wgc_capturer_win>(
      options, std::make_unique<wgc_capture_source_factory>(),
      std::make_unique<source_enumerator>(options.enumerate_current_process_windows()), false);
}
...
void wgc_capturer_win::capture_frame() {
  ...
  int capture_time_ms = (time_nanos() - capture_start_time_nanos) / k_num_nanosecs_per_millisec;
