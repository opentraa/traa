...
thread_desktop *thread_desktop::get_desktop(const WCHAR *desktop_name) {
  ...
  if (desk == NULL) {
    LOG_ERROR("Failed to open the thread_desktop '{}': {}", desktop_name, ::GetLastError());
    return nullptr;
  }
...
thread_desktop *thread_desktop::get_thread_desktop() {
  ...
  if (desk == NULL) {
    LOG_ERROR("Failed to retrieve the handle of the thread_desktop assigned to the current thread: {}",
              ::GetLastError());
    return nullptr;
  }
