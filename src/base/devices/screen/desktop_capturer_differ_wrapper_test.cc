...
#include "base/devices/screen/fake_desktop_capturer.h"
#include "base/devices/screen/mock_desktop_capturer_callback.h"
...
template <template <typename, typename...> class T = std::initializer_list, typename... Rect>
void assert_updated_region_covers(const desktop_frame &frame, const T<desktop_rect, Rect...> &rects) {
  desktop_region region;
  for (const auto &rect : rects) {
    region.add_rect(rect);
  }
...
void execute_differ_wrapper_case(black_white_desktop_frame_painter *frame_painter,
                              ...
                              bool exactly_match) {
  EXPECT_CALL(*callback, on_capture_result_ptr(desktop_capturer::capture_result::success, ::testing::_))
      .Times(1)
  ...
  execute_differ_wrapper_case(
      ...
      {desktop_rect::make_ltrb(0, 0, 40, 40),
       ...
       desktop_rect::make_ltrb(frame_generator.size()->width() - 40,
                             frame_generator.size()->height() - 40, frame_generator.size()->width(),
                             frame_generator.size()->height())},
      check_result, updated_region_should_exactly_match);

  base::random rd(std::chrono::system_clock::now().time_since_epoch().count());
  // Fuzzing tests.
...
TEST(desktop_capturer_differ_wrapper_test, DISABLED_capture_without_hints_perf) {
  int64_t started = std::chrono::system_clock::now().time_since_epoch().count();
  execute_differ_wrapper_test(false, false, false, false);
  ASSERT_LE(std::chrono::system_clock::now().time_since_epoch().count() - started, 15000);
}
