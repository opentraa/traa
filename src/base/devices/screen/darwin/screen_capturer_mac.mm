...
CFArrayRef create_window_list_with_exclusion(CGWindowID window_to_exclude) {
  ...
  for (CFIndex i = 0; i < CFArrayGetCount(all_windows); ++i) {
    ...
    CGWindowID id = get_window_id(window);
...
bool screen_capturer_mac::init() {
  LOG_EVENT("SDM", "screen_capturer_mac::Init");
...
void screen_capturer_mac::start(capture_callback *callback) {
  ...
  LOG_EVENT("SDM", "screen_capturer_mac::start, target display id {}", current_display_);
...
void screen_capturer_mac::capture_frame() {
  LOG_EVENT("SDM", "screen_capturer_mac::capture_frame");
...
void screen_capturer_mac::set_excluded_window(win_id_t window) { excluded_window_ = window; }
...
bool screen_capturer_mac::cg_blit(const desktop_frame &frame, const desktop_region &region) {
  ...
  for (size_t i = 0; i < displays_to_capture.size(); ++i) {
    ...
    if (excluded_image) {
      ...
      rect_to_copy.intersect_with(desktop_rect::make_wh(CGImageGetWidth(excluded_image.get()),
    ...
    CGDisplayStreamFrameAvailableHandler handler =
        ^(CGDisplayStreamFrameStatus status, uint64_t /* display_time */,
          ...
          if (count != 0) {
            ...
            screen_refresh(display_id, count, rects, display_origin, frame_surface);
          }
        };
