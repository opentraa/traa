...
void window_capturer_mac::capture_frame() {
  LOG_EVENT("SDM", "window_capturer_mac::capture_frame");
  ...
  if (full_screen_window_detector_) {
    ...
    CGWindowID full_screen_window =
        full_screen_window_detector_->find_full_screen_window(window_id_);
    ...
    if (full_screen_window != kCGNullWindowID) {
      ...
      if (!fullscreen_usage_logged_) {
        LOG_EVENT("SDM", "window_capturer_mac::capture_frame: full screen window detected");
