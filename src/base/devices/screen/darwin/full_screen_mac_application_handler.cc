...
public:
  ...
  full_screen_mac_application_handler(desktop_capturer::source_id_t source_id,
      ...
      : full_screen_app_handler(source_id), title_predicate_(title_predicate),
        owner_pid_(get_window_owner_pid(source_id)),
...
protected:
  ...
  find_full_screen_window_with_same_pid(const desktop_capturer::source_list_t &source_list_t,
    ...
    invalidate_cache_if_needed(source_list_t, timestamp, [&](const desktop_capturer::source_t &src) {
      return src.id != get_source_id() && get_window_owner_pid(src.id) == owner_pid_;
    });
    if (cache_sources_.empty())
      return kCGNullWindowID;

    const auto original_window = get_source_id();
    const std::string title = get_window_title(original_window);
    ...
    const auto it = std::find_if(cache_sources_.begin(), cache_sources_.end(),
                                 [&](const desktop_capturer::source_t &src) {
                                   const std::string window_title = get_window_title(src.id);
                                   ...
                                   return is_window_full_screen(desktop_config, src.id);
                                 });
  ...
  find_full_screen_window(const desktop_capturer::source_list_t &source_list_t,
    ...
    return !ignore_original_window_ && is_window_on_screen(get_source_id())
               ? 0
...
public:
  ...
  find_full_screen_window(const desktop_capturer::source_list_t &source_list_t,
                          int64_t timestamp) const override {
    invalidate_cache_if_needed(source_list_t, timestamp, [&](const desktop_capturer::source_t &src) {
      return get_window_owner_pid(src.id) == owner_pid_;
    });

    const auto original_window = get_source_id();
    const std::string original_title = get_window_title(original_window);
    ...
    const auto slide_show_window =
        std::find_if(cache_sources_.begin(), cache_sources_.end(), [&](const auto &src) {
          return src.title.empty() && is_window_full_screen(desktop_config, src.id);
        });
...
create_full_screen_app_handler(desktop_capturer::source_id_t source_id) {
  ...
  int pid = get_window_owner_pid(source_id);
  ...
  if (path_length > 0) {
    ...
    const std::string owner_name = get_window_owner_name(source_id);
