...
bool partial_block_difference(const uint8_t *old_buffer, const uint8_t *new_buffer, int width,
  ...
  for (int i = 0; i < height; i++) {
    if (std::memcmp(old_buffer, new_buffer, width_bytes) != 0) {
      return true;
    }
    old_buffer += stride;
    new_buffer += stride;
...
void desktop_capturer_differ_wrapper::on_capture_result(
  ...
  int64_t start_time_nanos = get_time_nanos();
  if (!input_frame) {
  ...
  constexpr int64_t k_num_millisec_per_second = INT64_C(1000);
  constexpr int64_t k_num_nanosecs_per_second = INT64_C(1000000000);
  constexpr int64_t k_num_nanosecs_per_millisec =
      k_num_nanosecs_per_second / k_num_millisec_per_second;

  frame->set_capture_time_ms(frame->capture_time_ms() +
                             (get_time_nanos() - start_time_nanos) / k_num_nanosecs_per_millisec);
