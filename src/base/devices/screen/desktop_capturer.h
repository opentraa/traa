...
public:
  ...
  public:
    ...
    virtual void OnCaptureResult(Result result, std::unique_ptr<desktop_frame> frame) = 0;
  ...
  // implementations.
  virtual void SetSharedMemoryFactory(std::unique_ptr<shared_memory_factory> shared_memory_factory);
...
protected:
  ...
  // capture windows.
  static std::unique_ptr<desktop_capturer>
  CreateRawWindowCapturer(const DesktopCaptureOptions &options);

  // Creates a platform specific desktop_capturer instance which targets to
  // capture screens.
  static std::unique_ptr<desktop_capturer>
  CreateRawScreenCapturer(const DesktopCaptureOptions &options);
};
