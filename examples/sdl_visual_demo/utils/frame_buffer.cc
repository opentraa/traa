...
void frame_buffer::push(const traa_video_frame *frame) {
  ...
  size_t data_size = static_cast<size_t>(w) * h * 3 / 2;

  // Use data_length if available and valid, otherwise calculate from I420 layout
  if (frame->data_length > 0 &&
      static_cast<size_t>(frame->data_length) >= data_size) {
...
bool frame_buffer::pop_to_texture(SDL_Renderer *renderer, SDL_Texture **texture,
  ...
  // Build a temporary traa_video_frame pointing to the local copy
  ...
  SDL_Texture *new_texture = frame_converter::i420_to_texture(renderer, &temp_frame);
  if (!new_texture) {
    return false;
  }
